...
const TimeDomain = ({ isDashboard = false, view }) => {
  ...
  useEffect(() => {
    if (sensorData.length > 0) {
      const updatedChartData = sensorData.map((item) => ({
        x: item.timestamp,
        xAxis: parseFloat(item.xAxis),
        yAxis: parseFloat(item.yAxis),
        zAxis: parseFloat(item.zAxis),
      }));
  ...
  const getSeriesData = () => {
    ...
    if (view === "xAxis") {
      seriesData.push({
        id: "xAxis",
        data: chartData.map((item) => ({ x: item.x, y: item.xAxis })),
      });
    }
    if (view === "yAxis") {
      seriesData.push({
        id: "yAxis",
        data: chartData.map((item) => ({ x: item.x, y: item.yAxis })),
      });
    }
    if (view === "zAxis") {
      seriesData.push({
        id: "zAxis",
        data: chartData.map((item) => ({ x: item.x, y: item.zAxis })),
      });
    }
    return seriesData;
  };
